...
class instructuretoimgcp():
    def __init__(self, api_token):
        ...
        self.courseids = {409805: "AP Calculus BC", 418634: "Computer Science A", 410143: "AP Physics", 419043: "English Honours", 410268: "AP World History"} 
                ...
                for assignmentData in json.loads(await req.read()):
                    ...
                    if not assignment["locked_for_user"] and int(assignment['points_possible']) != 0: # add not later
                        ...
                        try:
                            ...
                            try:
                                due = datetime.datetime.strptime(assignment['due_at'], "%Y-%m-%dT%H:%M:%Sz")
                            except:
                                due = None
                    ...
                    if parsedAssignments:
                        courseName = self.courseids[await self.stripName(assignmentData["course_id"])]
    ...
    def resize(self, monitor, img): # vs -> virtualScreen, resizes image to FILL mode
        ...
        if img.size != (width, height):
            ...
            bWidth, bHeight = img.size # Background image
            return img.crop(list(map(int, (crop, 0, bWidth-crop, height)))) # left | top | right | bottom
        else: return img
    ...
    def toPIL(self, courses):
        ...
        def drawTable(height=180):
            ...
            w = draw.textlength("Due Date", font=self.boldfontTitle) # Get width of "Due Date"
            draw.text(((self.W-x1-w)/2+x1, height), "Due Date", fill='black', font=self.boldfontTitle) # Add "Due Date" text
            w = draw.textlength("Points", font=self.boldfontTitle) # Get width of "Points"
            draw.text(((x2-x1-w)/2+x1, height), "Points", fill='black', font=self.boldfontTitle) # Add "Points" text
            w = draw.textlength("Assignments", font=self.boldfontTitle) # Get width of "Assignments"
    ...
    def background(self, img):
        if self.parsedcourses: # uncomment later
            ...
            # Coming back to this single line of code, longest 6 hours debugging errors
...
@app.route("/updatewallpaper", methods=["GET"])
async def main():
    async with aiohttp.ClientSession() as session:
        begin = recordTime("TOTAL TIME")
        rt = recordTime("FETCHING...")
        iti = instructuretoimgcp(os.getenv("TOKEN"))
        await iti.parse()
        rt.endTimer()
